...
async function setupWallets(client: SolanaClient) {
    try {
        ...
        // Generate a NEW test user for this attestation
...
async function main() {
    console.log("Creating Real SAS Attestation for Token Minting");
    console.log("===============================================");
    ...
    // Step 7: Save attestation info for minting 
    console.log("\n7. Saving attestation info for minting...");
    ...
    fs.writeFileSync('real-attestation-for-minting.json', JSON.stringify(attestationInfo, null, 2));
    console.log(`    - Attestation info saved to: real-attestation-for-minting.json`);
    console.log(`    - User: ${testUser.address}`);
    console.log(`    - Attestation: ${attestationPda}`);
    console.log("\n🎯 Real attestation ready for token minting!");
}

main()
    .then(() => console.log("\n✅ Real SAS attestation created successfully!"))
    .catch((error) => {
