...
async function main() {
  ...
  // Funded payer (issuer/authority)
  const payerRaw = fs.readFileSync('./funded-keypair.json', 'utf8');
  const payer = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(payerRaw)));

  // Generate a fresh user keypair and save it
  const user = Keypair.generate();
  fs.writeFileSync('./json/attested-user.json', JSON.stringify(Array.from(user.secretKey)));
  ...
  console.log('Created attestation for new user:', user.publicKey.toBase58(), 'sig:', sig);

  const out = {
    user: { address: user.publicKey.toBase58(), privateKey: Array.from(user.secretKey) },
    attestation: {
      pda: (attestationPda as any).toString(),
      credential: credentialPda.toBase58(),
      schema: schemaPda.toBase58(),
      nonce: user.publicKey.toBase58(),
  ...
  console.log('Wrote real-attestation-for-minting.json for new user');
}
